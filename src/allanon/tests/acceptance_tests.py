...
class AllanonTest(unittest.TestCase):
    ...
    def _get_downloaded_files(self):
        return sorted(listdir(self.temp_dir))
    ...
    def simple_multiple_resource_download_test(self):
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/foo%201.pdf",
            body=self._read_file("text1.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/foo2.pdf?aaa=b",
    ...
    def dynamic_url_downloads_test(self):
        ...
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/foo1.pdf?aaa=6",
            body=self._read_file("text1.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/foo1.pdf?aaa=7",
            body=self._read_file("text1.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/foo2.pdf?aaa=6",
            body=self._read_file("text2.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/foo2.pdf?aaa=7",
            body=self._read_file("text2.txt"),
        )
        main(self.options, "http://foo.org/foo{1:2}.pdf?aaa={6:7}")
        self.assertEqual(
            self._get_downloaded_files(),
            ["1-foo1.pdf", "2-foo1.pdf", "3-foo2.pdf", "4-foo2.pdf"],
        )
    ...
    def dynamic_directory_generation_test(self):
        self.options.destination_directory = os.path.join(
            self.temp_dir, "%HOST/series-%1"
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/bar-1/file.pdf",
            body=self._read_file("text1.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/bar-2/file.pdf",
            body=self._read_file("text1.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://baz.net/bar-1/file.pdf",
            body=self._read_file("text1.txt"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://baz.net/bar-2/file.pdf",
            body=self._read_file("text1.txt"),
        )
        main(
            self.options,
            "http://foo.org/bar-{1:2}/file.pdf",
            "http://baz.net/bar-{1:2}/file.pdf",
        )
        self.assertEqual(self._get_downloaded_files(), ["baz.net", "foo.org"])
        self.assertEqual(
            sorted(listdir(os.path.join(self.temp_dir, "baz.net"))),
            ["series-1", "series-2"],
        )
        self.assertEqual(
            sorted(listdir(os.path.join(self.temp_dir, "foo.org"))),
            ["series-1", "series-2"],
        )
    ...
    def test_duplicate_download(self):
        HTTPretty.register_uri(
            HTTPretty.GET, "http://foo.org/foo.pdf", body=self._read_file("text1.txt")
        )
        main(self.options, "http://foo.org/foo.pdf", "http://foo.org/foo.pdf")
        self.assertEqual(sorted(listdir(self.temp_dir)), ["foo.pdf", "foo_1.pdf"])
    ...
    def inner_resources_download_test(self):
        ...
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/section-1/download.html",
            body=self._read_file("page.html"),
        )
        HTTPretty.register_uri(
            HTTPretty.GET,
            "http://foo.org/section-2/download.html",
