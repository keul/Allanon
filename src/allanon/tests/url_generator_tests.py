...
class UrlGeneratorTest(unittest.TestCase):
    ...
    def test_range(self):
        ...
        self.assertEqual(results, [("foo 3 bar", [3]),
                                   ("foo 4 bar", [4]),
                                   ("foo 5 bar", [5])])

    def test_revert(self):
        results = list(generate_urls("foo {4:2} bar"))
        self.assertEqual(results, [("foo 4 bar", [4]),
                                   ("foo 3 bar", [3]),
                                   ("foo 2 bar", [2])])

    def test_multiple(self):
        results = list(generate_urls("foo {3:4} bar {39:41} baz"))
        self.assertEqual(results, [("foo 3 bar 39 baz", [3, 39]),
                                   ("foo 3 bar 40 baz", [3, 40]),
                                   ("foo 3 bar 41 baz", [3, 41]),
                                   ("foo 4 bar 39 baz", [4, 39]),
                                   ("foo 4 bar 40 baz", [4, 40]),
                                   ("foo 4 bar 41 baz", [4, 41])])
