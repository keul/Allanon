...
class ResourceGrabber(object):
    ...
    def _open(self):
        if self.request is None:
            ...
            try:
                ...
                while choice not in ("s", "t"):
                    choice = (
                        input(
                            "\nYou want to: "
                            "[s]kip this resource or [t]terminate (default)? "
                        )
                        .strip()
                        .lower()
                    )
            ...
            if self.request.status_code >= 200 and self.request.status_code < 300:
                ...
                print(
                    (
                        "Can't get resource at %s. HTTP error %d"
                        % (self.url, self.request.status_code)
                    )
                )

    def _get_filename(
        self, filename_model=None, ids=[], index=0, ids_digit_len=0, index_digit_len=0
    ):
        ...
        if filename_model:
            filename = self._generate_filename_from_model(
                filename,
    ...
    def _string_interpolation(
        self, model, ids=[], index=0, ids_digit_len=[], index_digit_len=0
    ):
        ...
        while dynaid_re.search(model):
            ...
            dynaid = match.group()
            model = model.replace(dynaid, _int_format(ids[cnt], ids_digit_len[cnt]), 1)
            cnt += 1
    ...
    def _generate_filename_from_model(
        self,
        original,
        filename_model,
        ids=[],
        ...
        filename = self._string_interpolation(
            filename_model, ids, index, ids_digit_len, index_digit_len
        )
        ...
        if filename.find("%EXTENSION") > -1:
            filename = filename.replace(
                "%EXTENSION", original[original.rfind(".") + 1 :]
            )
        # replace %EXTENSION with original extension
    ...
    def _create_subdirs(
        self, directory, ids=[], index=0, ids_digit_len=[], index_digit_len=0
    ):
        ...
        """
        directory = self._string_interpolation(
            directory, ids, index, ids_digit_len, index_digit_len
        )
    ...
    def _get_resource_content(self, file_out, filename):
        ...
        if size:
            progress = Bar(
                "Getting %s" % filename, fill="#", suffix="%(percent)d%%", max=size
            )
        else:
    ...
    def download(
        ...
        if not self.request:
            return
        directory = self._create_subdirs(
            directory,
            ids=ids,
            index=index,
            ids_digit_len=ids_digit_len,
        ...
        if duplicate_check and os.path.exists(path):
            ...
            if md5_saved == md5_remote:
                ...
                print(
                    (
                        "%d - Resource at %s is a duplicate of %s"
                        % (index, self.url, path)
                    )
                )
                return
